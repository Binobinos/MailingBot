...
async def handle_broadcast_text_interval(event: __CallbackQuery) -> None:
    ...
    async with bot.conversation(event.sender_id) as conv:
        ...
        new_broadcast_text_event = await conv.wait_event(event.NewMessage(from_users=event.sender_id))
        new_broadcast_text = new_broadcast_text_event.text

        await event.respond("⏳ Пожалуйста, отправьте интервал рассылки в минут.")
        try:
            new_interval_minutes_event = await conv.wait_event(event.NewMessage(from_users=event.sender_id))
